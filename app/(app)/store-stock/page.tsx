...
import { getProducts, getStoreStock } from "@/lib/data";
import { updateStoreStock } from "@/app/actions";
...
export default async function StoreStockPage() {
  await requireUser();
  const [products, stockRows] = await Promise.all([getProducts(), getStoreStock()]);
  ...
  return (
    <StockEditor products={items} onSave={updateStoreStock} title="Store stock" />
  );
}
